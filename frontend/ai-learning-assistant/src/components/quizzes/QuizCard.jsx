            ...
            <div>
                <h3
                  ...
                  <p className='text-xs font-medium text-slate-500 uppercase tracking-wide'>
                    Created {moment(quiz.createdAt).format("MM D, YYYY")}
                  </p>
            </div>
           ...
           <div className='mt-2 pt-4 border-t border-slate-100'>
            {quiz?.userAnswers?.length>0 ? (
               ...
               <Link to= {`/quizzes/${quiz._id}`}>
                <button className='group/btn relative w-full h-full bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden'>
                    <span className='relative z-10 flex items-center justify-center gap-2'>
                        <Play className='w-4 h-4' strokeWidth={2.5} />
                        Start Quiz
                    </span>
                    <div className='bg-linear inset-0 bg-linear-to-r from-white/0 via-white/20 to-white/0 -translate-x-full group-hover/btn:translate-x-full transition-transform duration-700'/>
                </button>
               </Link> 
            )}
           </div>
